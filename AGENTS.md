...
- **Auth env priority**: providers with `AuthEnv: ANTHROPIC_AUTH_TOKEN` (currently `minimax-cn`, `minimax-global`, `deepseek`, `xiaomimimo-cn`, `ollama`, and `ollama-cloud`) write bearer tokens; providers without `AuthEnv` write to `ANTHROPIC_API_KEY`. Both keys are in `managedEnvKeys` and get cleared on switch to avoid duplicates.
